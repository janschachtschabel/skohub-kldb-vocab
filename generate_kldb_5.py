#!/usr/bin/env python3
"""
KldB-5-Ebenen TTL Generator Script

Dieses Script erstellt eine vollständige hierarchische KldB-TTL mit allen 5 Ebenen
...
Usage: python enrich_kldb_ttl_5.py
"""
...
class KldB5EbenenGenerator:
    ...
    def parse_csv_data(self) -> bool:
        ...
        print("[INFO] Parse CSV-Daten und baue Hierarchie für alle 5 Ebenen...")
        
        # Filtere alle Ebenen 1-5
        filtered_data = self.csv_data[
            (self.csv_data['ebene'].astype(int) >= 1) & 
            (self.csv_data['ebene'].astype(int) <= 5)
        ].copy()
        
        print(f"[INFO] Gefilterte Daten: {len(filtered_data)} Einträge (Ebenen 1-5)")
    ...
    def get_parent_id(self, kldb_id: str, ebene: int) -> Optional[str]:
        """Bestimme Parent-ID basierend auf KldB-Hierarchie für alle 5 Ebenen"""
        if ebene <= 1:
            return None
...
\tdct:title "Klassifikation der Berufe - KldB 5-stellig (vollständig)"@de ;
\tdct:description "Vollständige hierarchische KldB mit allen 5 Ebenen und Kurzbeschreibungen aus KldB 2010 V. 2020"@de ;
...
"""
    ...
    def generate_ttl(self, output_file: str) -> bool:
        ...
        try:
            with open(output_file, 'w', encoding='utf-8') as f:
                ...
                for ebene in range(1, 6):  # Ebenen 1-5
                    concepts_at_level = [
    ...
    def print_statistics(self):
        """Drucke Statistiken"""
        print("\n" + "="*60)
        print("STATISTIKEN - ALLE 5 EBENEN")
        print("="*60)
        ...
        # Beispiele für tiefste Ebene (Ebene 5)
        ebene_5_concepts = [(cid, c) for cid, c in self.concepts.items() if c['ebene'] == 5]
        if ebene_5_concepts:
            print(f"\nBeispiele Ebene 5 (erste 5):")
            for i, (concept_id, concept) in enumerate(sorted(ebene_5_concepts)[:5]):
                print(f"  {concept_id}: {concept['titel']}")


def main() -> bool:
    """Hauptfunktion"""
    print("KldB-5-Ebenen TTL Generator")
    print("="*50)
    ...
    generator = KldB5EbenenGenerator()
    ...
    print(f"\n[SUCCESS] KldB-5-Ebenen.ttl erfolgreich erstellt!")
